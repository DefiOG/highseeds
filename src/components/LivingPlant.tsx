...
export function LivingPlant(props: PlantAppearance & { name?: string; preview?: boolean; archived?: boolean }) {
  const current = growthMilestone(props.progress);
  return <figure className="living-plant">
    <div className="living-plant-portrait">
      <svg viewBox="0 0 160 180" role="img" aria-label={`${props.name ?? 'Seed'}: ${props.failed ? 'failed crop' : current.name}, water ${Math.round(props.water ?? 100)} percent`}>
