...
import { PlantTraining } from './PlantTraining';
import { PlantHistory } from './PlantHistory';
import { visualGrowth, plantDamage } from '../lib/plantGrowth';
...
export function FarmWorld(props: Props) {
  const { trainPlant, state, now, connectWallet, activateNft, buyPlot, openPosition, careForPosition, closePosition, fulfillOrder, equipFarmer, setAutoHarvest, blocked } = props;
  ...
  const crops: CropVisual[] = Array.from({ length: WORLD_PAGE_SIZE }, (_, index) => {
    ...
    const color = ['#af97c3', '#a9bd77', '#bc91aa', '#89b6a4'][(credential?.tokenId ?? 0) % 4];
    return { occupied: Boolean(item?.position), available: Boolean(item), training: item?.position?.training, damage: item?.position ? plantDamage(item.position, now) : 0, seed: credential?.tokenId ?? 1, progress: item?.position && m ? visualGrowth(item.position.startedAt, m.endsAt, now, m.failed, m.progress) : 0, water: m?.waterLevel ?? 100, failed: m?.failed ?? false, color };
  });
