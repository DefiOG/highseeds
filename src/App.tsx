...
import { FAST_TIME_ENABLED, formatCountdown, LOCK_STEP_MS } from './lib/clock';
import {
  autoWaterReserve,
  elapsedSteps,
  formatNumber,
...
const LAND_DESCRIPTIONS: Partial<Record<PlotTierKey, string>> = {
  ...
  farm: 'Whale-scale commercial capacity, hard-capped at 36 live positions.',
};
...
export default function App() {
    ...
    setConfirm({
      ...
      onConfirm: () => {
        ...
        setState((current) => ({
          ...
          activity: [createActivity(`Plot #${id} acquired`, `${tier.label} · ${tier.slots} functional position slots.`), ...current.activity],
        }));
        setConfirm(null);
  ...
  const closePosition = (position: Position) => {
    ...
    if (metrics.completedSteps < 1) return notify('One checkpoint minimum', `Available in ${formatCountdown(position.startedAt + LOCK_STEP_MS - now)}.`, 'warning');
    ...
    setConfirm({
      ...
      lines: [
        ...
        ...(position.mode === 'worker' ? [{ label: 'Owner ledger', value: `${formatNumber(ownerPayout)}g` }] : []),
      ...
      onConfirm: () => {
        setState((current) => ({
          ...
          activity: [createActivity(metrics.mature ? `${formatNumber(payout)}g harvested` : `${formatNumber(payout)}g early settlement`, `${strain.name} · +${metrics.xpEarned} Access XP${ownerPayout ? ` · ${formatNumber(ownerPayout)}g owner ledger` : ''}.`), ...current.activity],
        }));
        setConfirm(null);
  ...
  return (
    <div className="app-shell">
      ...
      <div className="simulation-banner"><div><span className="status-dot" /> Pre-launch entertainment simulation <span className="banner-divider" /> No contracts · No real funds · No promised returns</div><div className="network-label"><ShieldCheck size={14} /> Economy v2</div></div>
      <aside className={mobileNav ? 'sidebar open' : 'sidebar'}>
        ...
        <div className="protocol-card"><div className="protocol-card-top"><Trophy size={15} /><span>{season.id} · Day {season.day}/{season.totalDays}</span></div><strong>{formatNumber(state.seasonXp, 0)} season XP</strong><p>Demand and leaderboards reset. Ownership and Access XP do not.</p><div className="protocol-progress"><span style={{ width: `${(season.day / season.totalDays) * 100}%` }} /></div></div>
...
function WorkView({ state, openPosition, go }: ViewContext) {
  ...
  const strain = STRAINS.find((item) => item.id === strainId)!;
  return <><PageHeading eyebrow="Landless path · fair split" title="Go to work." description="Use an open plot without buying land. The v2 test split is 50% worker / 50% owner, with estimated output shown before joining." /><OwnedLandOverview state={state} go={go} placement="work" /><div className="section-heading work-board-heading"><div><h2>Available work contracts</h2><p>Demo listings from other operators. Selecting one never consumes your owned land.</p></div><span className="count-chip">{externalPlots.length} listings</span></div><section className="work-grid"><div className="panel work-list">{externalPlots.map((item) => { const tier = PLOT_TIERS[item.tier]; const used = plotPositions(state, item.id).length; return <button key={item.id} className={plotId === item.id ? 'work-listing selected' : 'work-listing'} onClick={() => setPlotId(item.id)}><span className="work-land-thumb"><img src={LAND_ART[item.tier]} alt="" /></span><span><strong>{tier.label} #{item.id}</strong><small>{item.owner} · {item.reliability}% reliability</small></span><span><strong>{Math.max(0, tier.slots - used)} open</strong><small>{Math.round(WORKER_SHARE * 100)}% worker share</small></span></button>; })}</div><aside className="panel work-config"><p className="eyebrow">Work contract preview</p><h2>{PLOT_TIERS[plot.tier].label} #{plot.id}</h2><div className="work-selected-art"><img src={LAND_ART[plot.tier]} alt={`${PLOT_TIERS[plot.tier].label} operation`} /><span><strong>{plot.reliability}% reliable</strong><small>Select another operation to inspect its layout.</small></span></div><label>Access<select value={nftId} onChange={(event) => setNftId(Number(event.target.value))}>{nfts.map((item) => <option key={item.tokenId} value={item.tokenId}>#{item.tokenId} · {item.rarity}</option>)}</select></label><label>Strain<select value={strainId} onChange={(event) => setStrainId(event.target.value)}>{STRAINS.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.playstyle}</option>)}</select></label><label>Term<select value={duration} onChange={(event) => setDuration(event.target.value as DurationKey)}>{Object.entries(DURATION_PRESETS).map(([key, item]) => <option key={key} value={key}>{item.label} · {item.maturityMultiplier.toFixed(2)}×</option>)}</select></label><div className="work-estimate"><span>Estimated worker maturity</span><strong>{nft ? formatNumber(projectedMatureYield(nft, plot, strain.id, duration, 'worker')) : '—'}g</strong><small>Owner receives the matching 50% on the network owner ledger.</small></div><button className="button primary full" disabled={!nft} onClick={() => nft && openPosition(plot, nft, strain, duration, 'worker', true)}><UserRoundCheck size={16} /> Review work contract</button></aside></section></>;
}
